...
### DK-2026-08-06-05 — Moldflow sitewide kaldırıldı; sektör sayfalarında Markalar bölümü blogtan sonra tek-satır logo şeridine dönüştürüldü; mobilde 3+ öğeli şeritler 2 sütuna alındı

- **Yapan:** Onur, tek mesajda üç ayrı istek iletti: (1) mimarlık ürün filtresinde seçilemeyen bir filtre olduğunu bildirdi, (2) sektör sayfalarında Markalar bölümünün blogtan sonra, tek satırda ve sadece logo olarak gösterilmesini istedi, (3) (mesaj ortasında) mobilde 3'ten fazla öğeli her şeridin (örnek: Tandem "Temel Özellikler") 2 sütuna inmesini istedi, (4) (mesaj ortasında, ayrı bir talep) Autodesk Moldflow'un satılmadığını, siteden çıkarılmasını istedi.
- **1) Filtre bulgusu — tekrar üretilemedi:** `sektor_mimari.html`'deki 5 ürün filtresi (BIM & Tasarım/Görselleştirme/Koordinasyon & Saha/Çıktı & Baskı/Donanım) hem masaüstünde gerçek `.click()` ile hem mobildeki "Seçiniz" açılır listesinde (`mobilenav.js`'teki `.mfsel`/`.cbsel` bileşenleri) programatik olarak test edildi — 5'i de doğru şekilde aktifleşip ürünleri filtreliyor, örtüşen/tıklanamaz bir öğe bulunamadı (`elementFromPoint` ile üst-öğe kontrolü de her chip için kendisini döndürdü). 📋 **Kapsam dışı kaldı** — hangi filtre/sayfa/cihaz olduğu netleşmeden ek aksiyon alınmadı; ekran görüntüsü istendi.
- **2) Moldflow sitewide kaldırıldı (Onur: "satmıyoruz siteden çıkar"):** `cadbim_moldflow.html` ve `assets/logos/products/moldflow.svg` silindi. Referanslar temizlendi: `cadbim_urunler.html`, `cadbim_autodesk.html`, `cadbim_simulasyon.html` (pcard + cz-bul + .grid.g3 kartı + prose cümlesi), `sektor_havacilik.html` (pcard + card metni + marka satırı), `cadbim_endustriler.html` (havacilik ind-panel), `404.html` (slug→dosya haritası), `mobilenav.js` (site içi arama girdisi), `sitemap.xml` (`<url>` girdisi), `docs/htaccess-taslak.txt` (2 RewriteRule). Kendi önceki oturumumda eklediğim `sektor_tuketici_urunleri.html` (ürün kartı, iş akışı adımı, 1 SSS + JSON-LD) ve `scripts/gen_yapi_tuketici_sektor.py` (üretici script, yeniden çalıştırılırsa geri gelmesin diye) da düzeltildi. `assets/css/tpl-urun-d.css` başlık yorumu güncellendi.
- **3) Markalar bölümü — 11 sektör sayfası (`scripts/move_brands_after_blog.py`):** Önceki konum (Çözümler'den sonra, isim+ürün listesi içeren dikey `.brand-row` satırları) kaldırıldı; yerine Blog bölümünden sonra, CTA şeridinden önce, sadece logo içeren tek satırlık `.brand-strip` (marka adına göre ilgili ürün sayfasına linkli) kondu. Her sayfanın kendi `<style>` bloğuna `.brand-strip` CSS'i eklendi. Eski `.brand-row`/`.brand-stack` CSS tanımları dokunulmadan bırakıldı (artık kullanılmıyor ama zararsız).
- **4) Mobilde 3+ öğeli şeritler 2 sütuna iner — sitewide (`assets/css/mobile-guardrails.css`, R5 genişletildi):** Mevcut R5 kuralı sadece `.g3`/`.g4` sınıflarını kapsıyordu (zaten çalışıyordu); `auto-fit` ile mobilde 1 sütuna inen diğer "özellik şeridi" kalıpları da eklendi: `.feat-grid` (6 `tpl-urun-*.css` şablonunda, ör. Tandem'in "Temel Özellikler"i), `.cz-buls`/`.cz-fark`/`.cz-hat`/`.cz-steps` (design-system.css, çözüm sayfaları), `.srvgrid`/`.why-grid` (index.html), `.brand-stack`. **Cache-busting:** `mobile-guardrails.css?v=4`→`?v=5`, 1331 sayfada (script ile) — versiyon numarası değişmeden tarayıcı eski CSS'i önbellekten sunuyordu, canlıda da aynı sorun olurdu.
- **Doğrulama (localhost:8433):** Moldflow — sitewide `moldflow` araması yalnızca tarihsel değişiklik kayıtları/denetim raporlarında kaldı (kasıtlı, geçmiş kayıt bozulmaz). Markalar — 3 sayfa (`sektor_mimari` 5 logo, `sektor_tuketici_urunleri` 4 logo, `sektor_icmimarlik` 6 logo/Lumion dahil) `compareDocumentPosition` ile blog-sonrası/cta-öncesi konum ve tüm logo dosyaları `fetch` ile 200 OK doğrulandı; mobilde (375px) 6 logo doğal `flex-wrap` ile 2'şerli sarıyor. Mobil 2-sütun — `cadbim_tandem.html` `.feat-grid` (6 kart, mobil 161px×2 sütun / masaüstü 1280px'de 284px×4 sütun, aradaki fark yalnızca CSS versiyonu güncellenip cache-bust edildikten sonra göründü) ve `sektor_tesisat.html` `.cz-buls` (4 kart, mobil 2×2) canlı ölçüldü. CSP'nin `www.google.com.tr` konsol hataları (DK-91'de kabul edilmiş bilinen sınırlama) dışında hata yok.
- **Durum:** ✅ Moldflow kaldırma, Markalar taşıma, mobil 2-sütun tamamlandı. 📋 Filtre bulgusu — Onur'dan ek bilgi (ekran görüntüsü/sayfa/cihaz) bekleniyor.
...
- **Gap analizi:** Autodesk'in AECO/Manufacturing/M&E taksonomisiyle mevcut 9 sektör (Mimarlık, İç Mimarlık, İnşaat, Tesisat, Makine, Otomotiv, Havacılık, Medya, Eğitim) karşılaştırıldı. Building Products & Fabrication ve Consumer Products net boşluk (ayrı, çakışmayan pazar) bulundu; Industrial Equipment Manufacturing Makine sayfasıyla örtüştüğü için ayrı sayfa yerine kapsam genişletmesi önerildi; Electronics/Medical Device/Marine/Utilities reddedildi (Autodesk'in kendisi de bunları ayrı `/industry/` sayfası seviyesine çıkarmamış veya CADBİM'in bayi ağı için hedef kitle zayıf).
- **2 yeni sektör sayfası** (`sektor_yapi_urunleri.html`, `sektor_tuketici_urunleri.html`) — mevcut `sektor_otomotiv.html` iskeletinden üretildi (`scripts/gen_yapi_tuketici_sektor.py`, iki-tonlu 500/400 aksan renk şeması otomatik taşındı: turuncu f97316/fb923c ve mavi 3b82f6/60a5fa). Her sayfada: hero, 5 adımlı iş akışı, 7 çözüm kartı, marka/ürün kataloğu (Advance Steel, Nesting, Inventor, Fusion CAM / Alias, Moldflow, UltiMaker, Chaos V-Ray), 6 soruluk SSS + FAQPage JSON-LD (json.loads/dumps ile güvenli mutasyon). Bespoke SVG hero görseli (`scripts/gen_sektor_visuals.py`'ye `yapiurunleri()`/`tuketici()` eklendi — çelik bağlantı+nesting kesim ve enjeksiyon kalıp kesiti+render viewport) ve OG görseli (`scripts/gen_og_sektor.py`) üretildi.
- **Sitewide senkronizasyon** (`scripts/update_nav_11_sektor.py`, 1330+ sayfa: 205 kök + 1130 blog yazısı): Endüstriler nav dropdown + footer kolonu 11 sektöre çıkarıldı (blog yazılarındaki `../` önekli href'ler otomatik korundu). `mobilenav.js` (menü+arama listeleri), `index.html` (sektör seçici satırı + istatistik sayacı 9→11), `cadbim_endustriler.html` hub (kart/tab/panel + "9 Endüstri"→"11 Endüstri" metinleri), `sitemap.xml` (2 yeni `<url>`) güncellendi.
- **Makine & Üretim kapsam genişletmesi** (`sektor_makine.html`, minimal/hedefli): meta description + OG/twitter description + JSON-LD description'a "ağır ekipman ve endüstriyel makine üreticileri (Industrial Equipment Manufacturing) dahil" eklendi; hero badge "Manufacturing"→"Manufacturing & Industrial Equipment"; hero alt metni güncellendi. Sayfa yapısı değiştirilmedi.
- **Bilinçli kapsam dışı:** Yeni sayfalarda "Başarı Öyküleri" bölümü yok (gerçek müşteri referansı olmadığı için — mevcut proje kuralı, tesisat/icmimarlik'te de aynı kural uygulanmıştı).
- **Doğrulama (localhost:8433):** Her iki sayfa `get_page_text` ile uçtan uca okundu (hero→FAQ→footer→webinar widget'ı dahil tam render), `read_network_requests` ile tüm asset'ler (SVG, CSS, JS, blog-posts.json) 200 OK; JSON-LD `json.loads` ile doğrulandı; nav/footer'daki tek "otomotiv" referansının kasıtlı Otomotiv sektör linki olduğu (leftover metin değil) teyit edildi. Tek 404, clean-URL'nin yerel `dev_server.py`'de rewrite edilmemesi (bilinen, production'da sorun değil).
- **Not:** `gen_sektor_visuals.py`'nin `otomotiv()`/`havacilik()` fonksiyonları, canlıdaki gerçek SVG'lerden (daha sonra elle/başka oturumda güncellenmiş "aerodinamik akış" vb. detaylar içeriyor) geride kalmış durumda — script'i BUILDERS sözlüğü üzerinden toplu çalıştırmak bu 2 dosyayı eski içerikle ezer (bu oturumda tespit edilip `git checkout` ile geri alındı). İleride script'i çalıştıracak kişi için bilgi notu eklendi.
- **Durum:** ✅ Tamamlandı, commit b94543cc0 ile push edildi (main).
...
- **1) Footer'a açık adres (200 sayfa):** Denetim raporunun #10 bulgusu — footer'da sadece "İzmir Merkez Ofis / Ankara Temsilcilik" etiketi vardı, açık adres yalnız görünmeyen JSON-LD'de. Paylaşılan `.f-offices` bloğu (209 sayfada birebir aynı, sadece 171'i LF sonlu 32'si CRLF sonlu — iki geçişli script gerekti) İzmir satırının altına küçük punto (11px) bir alt satır olarak "Ataşehir Mah. Anadolu Cad. Kaşarcı Plaza No:901, Çiğli / İzmir" eklendi. Ankara'nın açık adresi yok (sadece temsilcilik/e-posta), ona dokunulmadı. Atlanan 3 dosya (404.html, cadbim_construction_cloud.html, cadbim_sanatsal_baski.html) farklı/minimal footer yapısına sahip — kasıtlı olarak dokunulmadı. Yerel önizlemede DOM ve layout (flex-column, taşma yok) doğrulandı.
- **2) CTA tutarlılığı (9 sayfa, seçici genişletme):** ~63 sayfa "Teklif İste" dışında bir kapanış CTA'sı kullanıyordu (Ücretsiz Deneme Talebi, İletişime Geç, Uzmanla Konuş, Uzmanımıza Sorun, Proje Görüşmesi, Eğitim Talebi Oluştur — script ile tüm site tarandı, `cta_scan.json`). Her grup incelenip **sadece gerçek tutarsızlık olan** iki alt küme düzeltildi:
  - 7 çözüm sayfası (bim, bim_icerik_uretimi, dijital_donusum, dijital_ikiz, gerceklik_yakalama, insaat_yonetimi, yaratici_icerik): kapanış CTA'sı "İletişime Geç" idi ve genel iletişim formuna (`iletisim#form`) gidiyordu — kardeş çözüm sayfalarının (cam, pdm, simulasyon, gorsellestirme vb., DK-966ea6c15'te düzeltilmiş) hepsi "Teklif İste" ile düşük sürtünmeli teklif formuna (`teklif-iste#form`) gidiyor. Aynı kalıba çekildi.
  - 2 sektör sayfası (sektor_egitim, sektor_havacilik): kapanış CTA'sı "İletişime Geç" idi, kendi şablon ailesindeki diğer 7 sektör sayfası "Uzmanla Konuş" kullanıyordu — şablon içi tutarlılık için etiket düzeltildi (href zaten aynıydı).
  - **Bilinçli dokunulmayanlar (farklı funnel/sayfa türü, tutarsızlık değil kasıtlı fark):** 7 sektör sayfasının "Uzmanla Konuş" kalıbı (farkındalık aşaması, ürün seçilmemiş); cozumler.html hub'ın "Uzmanımıza Danışın"; danismanlik/yazilim_gelistirme'nin "Proje Görüşmesi" (hizmet/proje çerçevesi, lisans teklifi değil); egitimler.html'in "Eğitim Talebi Oluştur" (farklı eylem — kayıt, teklif değil); hakkimizda/webinar/blog/surdurulebilirlik'in "İletişime Geç" (bilgilendirici sayfalar, sert satış CTA'sı uygun değil); 21 Autodesk ürün sayfasının (advance_steel, arnold, autocad_lt vb.) 3 butonlu "Ücretsiz Deneme Talebi / Abonelik Planları / İletişime Geçin" şablonu — hepsi aynı genel forma gidiyor ve gerçek bir deneme akışına bağlanmıyor, ama iş modeli netliği (Cadbim üzerinden gerçek bir "ücretsiz deneme" süreci var mı) teyit gerektirdiği için **dokunulmadı, ayrı onay için işaretlendi**.
- **3) cadbim_dwg_trueview.html — ücretsiz ürün/ücretli dil çelişkisi giderildi:** Sayfa "Lisanslama & Abonelik" başlıklı bir bölüm, "Esnek Abonelik Seçenekleri" kartı, "2. Lisanslama / Tedarik — resmî kanaldan, doğru fiyatla, kayıtlı sözleşmeyle temin" adımı ve "Ücretsiz deneme, abonelik planları ve kurumsal lisanslama" CTA metniyle ücretsiz bir Autodesk aracını ücretli ürün gibi anlatıyordu. Tüm bu bölümler "DWG TrueView ücretsizdir, Cadbim'in katkısı sürüm/uyumluluk rehberliği ve gerektiğinde AutoCAD'e geçiş danışmanlığıdır" çerçevesine yeniden yazıldı; 3 butonlu CTA'daki anlamsız "Abonelik Planları" butonu kaldırıldı (2 butona indi: Teklif İste + Uzmanımıza Sorun).
- **Doğrulama:** Tüm diff'ler (`git diff`) tek tek okundu; footer eklemesi tarayıcıda DOM/layout olarak doğrulandı; HTML etiket yapısı hiçbir dosyada bozulmadı.
- **Durum:** ✅ Tamamlandı (footer 200/203 sayfa, CTA 9/9 planlanan sayfa, dwg_trueview). 📋 Kapsam dışı bırakılan: 21 sayfalık "Ücretsiz Deneme" CTA grubu (iş modeli teyidi gerekiyor), footer'ın 3 istisna sayfası (kasıtlı farklı şablon).
